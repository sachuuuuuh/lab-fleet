...
$msiOut = Join-Path $root "release\lab-fleet-0.1.0-x64.msi"
...
& (Join-Path $wixDir "candle.exe") -nologo -arch x64 -ext WixUtilExtension -ext WixFirewallExtension "-dDesktopDir=$desktopDir" "-dAgentDir=$agentDir" "-dProductVersion=0.1.0" -out (Join-Path $wixOut "Product.wixobj") (Join-Path $root "packaging\windows\Product.wxs")
