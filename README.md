...
The MSI is written to `release/lab-fleet-0.1.3-x64.msi`. It includes a guided install/maintenance wizard and is unsigned unless signing credentials are supplied by the release environment. Build it locally once, then copy the same MSI to every Windows x64 lab computer.
...
```powershell
msiexec /i lab-fleet-0.1.3-x64.msi
```
...
```bash
sudo apt install ./lab-fleet-0.1.3-amd64.deb
```
